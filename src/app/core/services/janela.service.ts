...
export class JanelaService {
      ...
      BotaoSetEvent(botao:IBotoes){
        if (botao.tipoBotao == TipoBotao.apoioJanela)
          this.BotoesJanela(botao)
        if (botao.tipoBotao == TipoBotao.crud)
          this.BotoesCrud(botao)
      }
      BotoesJanela(botao:IBotoes){
         switch(this.Janela.buttons.find(b => b.tipoBotao == TipoBotao.apoioJanela && b.name == botao.name)?.evento){
          case "new":
            this.LimparJanela()
            break;
          case "cancel":
      ...
      BotoesCrud(botao:IBotoes){
        return this.Janela.buttons.find(b => b.tipoBotao == TipoBotao.crud && b.name == botao.name)?.evento
      }
